...
func (r Result) Human(w io.Writer, showExtendedMetadata, colored bool) {
	...
	for i, path := range r.Paths {
		...
		entries := cs.KeyValues(
			...
			"PQC-secured", fmt.Sprint(path.FullPath.Metadata().PQsecure),
		)
...
// humanSbom summarizes the SBOM information in the meta
// data in a human readable string. Returns empty string if no information is
// available.
func humanSbom(p *snet.PathMetadata) string {
	complete := true
	var tot int64 = 0
	for _, v := range p.Sbom {
		complete = complete && v >= 0
		if v >= 0 {
			tot += v
		}
