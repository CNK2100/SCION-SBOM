...
// symmetrizeSbom makes the Intra SBOM values symmetric
func symmetrizeSbom(sbom map[common.IFIDType]InterfaceSboms) {
	for i, sub := range sbom {
		delete(sub.Intra, i) // Remove loopy entry
		for j, v := range sub.Intra {
			if _, ok := sbom[j]; !ok {
				continue
			...
			if sbom[j].Intra == nil {
				sbom[j] = InterfaceSboms{
					Inter: sbom[j].Inter,
					Intra: make(map[common.IFIDType]uint64),
				}
			}
			vTransposed, ok := sbom[j].Intra[i]
			// Set if not specified or pick more conservative value if both are specified
			if !ok || v < vTransposed {
				sbom[j].Intra[i] = v
			}
...
// generateSbom creates the SbomInfo by extracting the relevant values
// from the config.
func (cfg StaticInfoCfg) generateSbom(ifType map[common.IFIDType]topology.LinkType,
	ingress, egress common.IFIDType) staticinfo.SbomInfo {

	si := staticinfo.SbomInfo{
		Intra: make(map[common.IFIDType]uint64),
		Inter: make(map[common.IFIDType]uint64),
	}
	for ifid, v := range cfg.Sbom[egress].Intra {
		if includeIntraInfo(ifType, ifid, ingress, egress) {
			si.Intra[ifid] = v
		}
	}
	for ifid, v := range cfg.Sbom {
		t := ifType[ifid]
		if ifid == egress || t == topology.Peer {
			si.Inter[ifid] = v.Inter
		}
	}
	// Return empty SbomInfo if no data (matching other fields' behavior)
	if len(si.Intra) == 0 && len(si.Inter) == 0 {
		return staticinfo.SbomInfo{}
	}
	return si
}
