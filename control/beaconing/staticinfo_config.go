...
// rbomcode: Define struct
type InterfaceRbomLevel struct {
	Inter float32              `json:"Inter"`
	Intra map[iface.ID]float32 `json:"Intra"`
}
...
type StaticInfoCfg struct {
	...
	// rbomcode: parse RbomLevel from config.json
	RbomLevel map[iface.ID]InterfaceRbomLevel `json:"RbomLevel"`
...
func (cfg *StaticInfoCfg) clean() {
	...
	// rbomcode: clean
	delete(cfg.RbomLevel, 0)
	for _, s := range cfg.RbomLevel {
		delete(s.Intra, 0)
	}
	...
	symmetrizeRbomLevel(cfg.RbomLevel) // rbomcode: symmetrize
...
// rbomcode: symmetrizeRbomLevel
// symmetrizeRbomLevel makes the Intra RbomLevel values symmetric
func symmetrizeRbomLevel(RbomLevel map[iface.ID]InterfaceRbomLevel) {
	for i, sub := range RbomLevel {
		delete(sub.Intra, i) // Remove loopy entry
		for j, v := range sub.Intra {
			if _, ok := RbomLevel[j]; !ok {
				continue
			}
			if RbomLevel[j].Intra == nil {
				RbomLevel[j] = InterfaceRbomLevel{
					Inter: RbomLevel[j].Inter,
					Intra: make(map[iface.ID]float32),
				}
			}
			vTransposed, ok := RbomLevel[j].Intra[i]
			// Set if not specified or pick more conservative value if both are specified
			if !ok || v < vTransposed {
				RbomLevel[j].Intra[i] = v
			}
		}
	}
}
...
func (cfg StaticInfoCfg) generate(ifType map[iface.ID]topology.LinkType,
	...
	return &staticinfo.Extension{
		...
		RbomLevel:    cfg.generateRbomLevel(ifType, ingress, egress), // rbomcode
...
// rbomcode: generateRbomLevel
// generateRbomLevel creates the RbomInfo by extracting the relevant values
// from the config.
func (cfg StaticInfoCfg) generateRbomLevel(ifType map[iface.ID]topology.LinkType,
	ingress, egress iface.ID) staticinfo.RbomLevelInfo {

	rl := staticinfo.RbomLevelInfo{
		Intra: make(map[iface.ID]float32),
		Inter: make(map[iface.ID]float32),
	}
	for ifid, v := range cfg.RbomLevel[egress].Intra {
		if includeIntraInfo(ifType, ifid, ingress, egress) {
			rl.Intra[ifid] = v
		}
	}
	for ifid, v := range cfg.RbomLevel {
		t := ifType[ifid]
		if ifid == egress || t == topology.Peer {
			rl.Inter[ifid] = v.Inter
		}
	}
	return rl
}
