...
// rbomcode (needs implementation)
// RbomLevel returns an arbitrary test rbom_level value between two interfaces.
// Analogous to Latency.
func (g *Graph) RbomLevel(a, b uint16) float32 {
	sameIA := (g.parents[a] == g.parents[b])
	if !sameIA && g.links[a] != b {
		panic("interfaces must be in the same AS or connected by a link")
	}

	return float32(a * b * 11939 % 5000) // Need to change value range
}
...
func generateStaticInfo(g *Graph, ia addr.IA, inIF, outIF uint16) *staticinfo.Extension {
	...
	// rbomcode
	rbomLevel := staticinfo.RbomLevelInfo{}
	if outIF != 0 {
		rbomLevel.Intra = make(map[iface.ID]float32)
		rbomLevel.Inter = make(map[iface.ID]float32)
		for ifid := range as.IfIDs {
			if ifid != outIF {
				rbomLevel.Intra[iface.ID(ifid)] = g.RbomLevel(ifid, outIF)
			}
			if ifid == outIF || g.isPeer[ifid] {
				rbomLevel.Inter[iface.ID(ifid)] =
					g.RbomLevel(ifid, g.links[ifid])
			}
		}
	}
