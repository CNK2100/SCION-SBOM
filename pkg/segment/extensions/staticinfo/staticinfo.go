...
type Extension struct {
	...
	RbomLevel    RbomLevelInfo // rbomcode
	Geo          GeoInfo
...
// rbomcode
// RbomLevelInfo is the internal repesentation of `rbom_level` in the
// StaticInfoExtension.
type RbomLevelInfo struct {
	Intra map[iface.ID]float32
	Inter map[iface.ID]float32
}
...
func FromPB(pb *cppb.StaticInfoExtension) *Extension {
	...
	return &Extension{
		...
		RbomLevel:    RbomLevelInfoFromPB(pb.RbomLevel), // rbomcode
...
// rbomcode
func RbomLevelInfoFromPB(pb *cppb.RbomLevelInfo) RbomLevelInfo {
	if pb == nil || len(pb.Intra) == 0 && len(pb.Inter) == 0 {
		return RbomLevelInfo{}
	}
	intra := make(map[iface.ID]float32, len(pb.Intra))
	for ifid, v := range pb.Intra {
		intra[iface.ID(ifid)] = v
	}
	inter := make(map[iface.ID]float32, len(pb.Inter))
	for ifid, v := range pb.Inter {
		inter[iface.ID(ifid)] = v
	}
	return RbomLevelInfo{
...
func ToPB(si *Extension) *cppb.StaticInfoExtension {
	...
	return &cppb.StaticInfoExtension{
		...
		RbomLevel:    RbomLevelInfoToPB(si.RbomLevel), // rbomcode
...
// rbomcode (may have to look at datatypes)
func RbomLevelInfoToPB(rli RbomLevelInfo) *cppb.RbomLevelInfo {
	if len(rli.Intra) == 0 && len(rli.Inter) == 0 {
		return nil
	}
	intra := make(map[uint64]float32, len(rli.Intra))
	for ifid, v := range rli.Intra {
		intra[uint64(ifid)] = v
	}
	inter := make(map[uint64]float32, len(rli.Inter))
	for ifid, v := range rli.Inter {
		inter[uint64(ifid)] = v
	}
	return &cppb.RbomLevelInfo{
		Intra: intra,
		Inter: inter,
	}
}
