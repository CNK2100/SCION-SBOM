...
const (
	...
	// rbomcode
	// RbomLevelUnset is the default value for a rbom level entry in
	// PathMetadata for which no value was announced.
	RbomLevelUnset float32 = -1
)
...
type PathMetadata struct {
	...
	// rbomcode
	// RbomLevel lists the rbom level between any two consecutive interfaces.
	// Entry i describes the value between interfaces i and i+1.
	// A negative value (RbomLevelUnset) indicates that the AS did not
...
func (pm *PathMetadata) Copy() *PathMetadata {
	...
	return &PathMetadata{
		...
		RbomLevel:    append(pm.RbomLevel[:0:0], pm.RbomLevel...),
