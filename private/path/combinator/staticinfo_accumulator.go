...
func collectMetadata(interfaces []snet.PathInterface, asEntries []seg.ASEntry) snet.PathMetadata {
	...
	return snet.PathMetadata{
		...
		RbomLevel:            collectRbomLevel(path),
		Geo:                  collectGeo(path),
...
func collectLatency(p pathInfo) []time.Duration {
	...
	for _, asEntry := range p.ASEntries {
		...
		egIF := snet.PathInterface{
			IA: asEntry.Local,
			ID: iface.ID(asEntry.HopEntry.HopField.ConsEgress),
		}
...
// rbomcode (may need implementation)
func collectRbomLevel(p pathInfo) []float32 {
	// This is identical to collecting latencies.
	// 1)
	hopRbomLevels := make(map[hopKey]float32)
	for _, asEntry := range p.ASEntries {
		staticInfo := asEntry.Extensions.StaticInfo
		if staticInfo == nil {
			continue
		...
		RbomLevel := staticInfo.RbomLevel
		// Egress to other local interfaces
		for ifid, v := range RbomLevel.Intra {
			otherIF := snet.PathInterface{IA: asEntry.Local, ID: ifid}
			addHopRbomLevel(hopRbomLevels, egIF, otherIF, v)
		}
		// Local peer to remote peer interface
		for ifid, v := range RbomLevel.Inter {
			localIF := snet.PathInterface{IA: asEntry.Local, ID: ifid}
			addHopRbomLevel(hopRbomLevels, localIF, p.RemoteIF[localIF], v)
		}
	}

	// 2)
	RbomLevels := make([]float32, len(p.Interfaces)-1)
	for i := 0; i+1 < len(p.Interfaces); i++ {
		vu, ok := hopRbomLevels[makeHopKey(p.Interfaces[i], p.Interfaces[i+1])]
		v := float32(vu)
		if !ok {
			v = snet.RbomLevelUnset
		}
		RbomLevels[i] = v
	}
...
// rbomcode (may need to keep lower value?)
// addHopRbomLevel adds the rbom_level of hop a-b to the map. Handle conflicting entries by
// chosing the more conservative value (i.e. keep higher value).
func addHopRbomLevel(m map[hopKey]float32, a, b snet.PathInterface, v float32) {
	// Skip incomplete entries; not strictly necessary, we'd just not look this up
	if a.ID == 0 || b.ID == 0 {
		return
	}
