...
type Path struct {
	...
	// rbomcode
	RbomLevel  []float32                        `json:"rbom_level"`
	...
	for i, path := range r.Paths {
		...
		if showExtendedMetadata {
			...
			entries = append(entries, filteredKeyValues(cs,
				...
				"RbomLevel", humanRbomLevel(meta), // rbomcode
...
// rbomcode (needs editing)
// humanRbomLevel summarizes the rbom_level information in the meta
// data in a human readable string. Returns empty string if no information is
// available.
func humanRbomLevel(p *snet.PathMetadata) string {
	complete := true
	var tot float32 = 0
	for _, v := range p.RbomLevel {
		complete = complete && v >= 0
		if v >= 0 {
			tot += v
		}
	...
	if complete {
		return fmt.Sprintf("%fgCO2/TB", tot)
	}
	if tot > 0 {
		return fmt.Sprintf(">%fgCO2/TB (information incomplete)", tot)
	}
	return ""
}
...
func Run(ctx context.Context, dst addr.IA, cfg Config) (*Result, error) {
	...
	for _, path := range paths {
		...
		rpath := Path{
			...
			RbomLevel:   pathMeta.RbomLevel, // rbomcode
			Hops:        []Hop{},
		}
